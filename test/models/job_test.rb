...
class JobTest < ActiveSupport::TestCase
  ...
  test 'parity_valid? should return false if gap is more than 15' do
    job = jobs(:job_2)
    assert_equal false, job.parity_valid?
  end

  test 'parity_valid? should return true if gap is less than or equal to 15' do
    job = jobs(:job_1)
    assert_equal true, job.parity_valid?
  end

  test 'parity_invalid? should return true if gap is more than 15' do
    job = jobs(:job_2)
    assert_equal true, job.parity_invalid?
  end

  test 'parity_invalid? should return false if gap is less than or equal to 15' do
    job = jobs(:job_1)
    assert_equal false, job.parity_invalid?
  end
